...
if (-not $isAdmin) {
    ...
    Write-Host ""
    if (-not $Silent) { Write-Host "Press any key to exit..." -ForegroundColor Gray; $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown') }
    exit 1
}

# ── Step 1: Extract files ──
Write-Step "[1/3] Extracting files to $InstallPath..."
...
Write-Step "[2/3] Creating shortcuts..."
...
# ── Step 3: Register in Add/Remove Programs ──
Write-Step "[3/3] Registering application..."
