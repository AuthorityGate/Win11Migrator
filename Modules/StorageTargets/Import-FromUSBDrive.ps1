<#
========================================================================================================
    Title:          Win11Migrator - USB Drive Import Handler
    Filename:       Import-FromUSBDrive.ps1
    Description:    Reads a migration package from a USB drive for import on the target machine.
...
<#
.SYNOPSIS
    Imports a migration package from a USB drive to a local temp directory.
.DESCRIPTION
    Locates the Win11Migrator migration package on the specified USB drive,
...
.PARAMETER USBDrivePath
    Path to the USB drive root or the specific migration package folder.
    If a drive root is given, the function searches for the Win11Migrator folder.
...
.EXAMPLE
    $imported = Import-FromUSBDrive -USBDrivePath "E:\"
    Read-MigrationManifest -ManifestPath $imported.ManifestPath
#>

function Import-FromUSBDrive {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$USBDrivePath,
    ...
    Write-MigrationLog -Message "Importing migration package from USB drive..." -Level Info

    # Validate USB path exists
    if (-not (Test-Path $USBDrivePath)) {
        throw "USB drive path not accessible: $USBDrivePath"
    }

    # Locate the migration package on the USB drive
    $packageSource = $null

    # Check if the given path IS the package (contains manifest.json)
    $directManifest = Join-Path $USBDrivePath 'manifest.json'
    if (Test-Path $directManifest) {
        $packageSource = $USBDrivePath
    ...
    if (-not $packageSource) {
        $migratorFolder = Join-Path $USBDrivePath 'Win11Migrator'
    ...
    # Broad search as last resort (one level deep from drive root)
    if (-not $packageSource) {
        $searchDirs = Get-ChildItem -Path $USBDrivePath -Directory -ErrorAction SilentlyContinue
    ...
    if (-not $packageSource) {
        throw "No migration package found on USB drive at $USBDrivePath. Expected a folder containing manifest.json."
    }
    ...
    if ($exitCode -ge 8) {
        throw "Robocopy import from USB failed with exit code $exitCode."
    }
    ...
    Write-MigrationLog -Message "USB import complete: $fileCount files, $totalSizeMB MB" -Level Success
    return $result
}
