...
function Push-MigrationDirect {
    ...
    $userDataItems = @()
    if ($State.ContainsKey('UserData')) {
        $userDataItems = @($State.UserData | Where-Object { $_.Selected })
    }
    ...
    try {
        ...
        # Copy full local export package subdirectories to remote package for CLI import
        $localPkgPath = $State.PackagePath
        if ($localPkgPath -and (Test-Path $localPkgPath)) {
            $subdirs = @('UserData', 'BrowserProfiles', 'AppData', 'AppProfiles')
            foreach ($subdir in $subdirs) {
                $localSubdir = Join-Path $localPkgPath $subdir
                if (Test-Path $localSubdir) {
                    $remoteSubdir = Join-Path $remotePackagePath $subdir
                    if (-not (Test-Path $remoteSubdir)) {
                        New-Item -Path $remoteSubdir -ItemType Directory -Force -ErrorAction Stop | Out-Null
                    }
                    $null = & robocopy "`"$localSubdir`"" "`"$remoteSubdir`"" /E /R:$retries /W:$waitSec /MT:$threads /NP /NDL /NJH /NJS /XJ 2>&1
                    $rcExit = $LASTEXITCODE
                    if ($rcExit -lt 8) {
                        Write-MigrationLog -Message "Copied $subdir to remote package (exit code: $rcExit)" -Level Info
                    } else {
                        Write-MigrationLog -Message "Failed to copy $subdir to remote package (exit code: $rcExit)" -Level Warning
                    }
                }
            }
        }
...
& (Join-Path `$migratorRoot 'Win11Migrator.ps1') -CLI import -PackagePath '$localPackagePath' -Silent
"@
        ...
        if (-not $remoteRestoreLaunched) {
            # Try CIM process creation as fallback
            try {
                ...
                $cimResult = Invoke-CimMethod -CimSession $cimSession -ClassName Win32_Process -MethodName Create `
                    -Arguments @{ CommandLine = $processArgs } `
                    -ErrorAction Stop
                if ($cimResult.ReturnValue -eq 0) {
                    $remoteRestoreLaunched = $true
                    Write-MigrationLog -Message "Restore launched on target via CIM (PID: $($cimResult.ProcessId))" -Level Success
                } else {
                    Write-MigrationLog -Message "CIM process creation returned code $($cimResult.ReturnValue)" -Level Warning
                }
                Remove-CimSession -CimSession $cimSession -ErrorAction SilentlyContinue
            } catch {
                Write-MigrationLog -Message "Could not launch restore via CIM: $($_.Exception.Message)" -Level Warning
            }
        }
