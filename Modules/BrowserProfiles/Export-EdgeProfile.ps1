<#
========================================================================================================
    Title:          Win11Migrator - Edge Profile Exporter
    Filename:       Export-EdgeProfile.ps1
    Description:    Exports Microsoft Edge bookmarks, extensions, settings, and history for migration.
...
<#
.SYNOPSIS
    Exports a Microsoft Edge browser profile (bookmarks, preferences, extensions list, history).
.DESCRIPTION
    Edge is Chromium-based and shares the same profile structure as Chrome.
...
function Export-EdgeProfile {
    ...
    if ($Profile.Browser -ne 'Edge') {
        Write-MigrationLog -Message "Export-EdgeProfile called with non-Edge profile: $($Profile.Browser)" -Level Warning
        $Profile.ExportStatus = 'Skipped'
        return $Profile
    }

    $profileName = $Profile.ProfileName -replace '[^\w\-\.]', '_'
    $destPath = Join-Path $OutputDirectory "BrowserProfiles\Edge\$profileName"
    $sourcePath = $Profile.ProfilePath

    Write-MigrationLog -Message "Exporting Edge profile '$($Profile.ProfileName)' from $sourcePath" -Level Info
    ...
    try {
        ...
        if (Test-Path $bookmarksFile) {
            ...
            Write-MigrationLog -Message "  Edge bookmarks exported" -Level Debug
            $exportedAny = $true
        }
        ...
        if (Test-Path $prefsFile) {
            ...
            Write-MigrationLog -Message "  Edge preferences exported" -Level Debug
            $exportedAny = $true
        }
        ...
        if (Test-Path $historyFile) {
            try {
                ...
                Write-MigrationLog -Message "  Edge history exported" -Level Debug
                $exportedAny = $true
            }
            catch {
                Write-MigrationLog -Message "  Edge history file is locked (browser may be running): $($_.Exception.Message)" -Level Warning
            }
        }
        ...
        if (Test-Path $extensionsDir) {
            ...
            foreach ($extFolder in $extFolders) {
                ...
                if ($versionDir) {
                    ...
                    if (Test-Path $manifestJson) {
                        try {
                            ...
                            Write-MigrationLog -Message "  Failed to parse Edge extension manifest for $extId" -Level Debug
                        }
                    }
                }
                ...
                $extensionList.Add([PSCustomObject]@{
                    ...
                    Version     = $extVersion
                    Description = $extDesc
                    EdgeAddOnUrl = "https://microsoftedge.microsoft.com/addons/detail/$extId"
                })
            }
            ...
            if ($extensionList.Count -gt 0) {
                ...
                Write-MigrationLog -Message "  Edge extensions list exported ($($extensionList.Count) extensions)" -Level Debug
                $exportedAny = $true
            }
        }

        # Explicitly do NOT export Login Data / passwords
        Write-MigrationLog -Message "  Edge Login Data (passwords) intentionally excluded for security" -Level Debug

        if ($exportedAny) {
            $Profile.ExportStatus = 'Success'
            Write-MigrationLog -Message "Edge profile '$($Profile.ProfileName)' export complete" -Level Success
        }
        else {
            $Profile.ExportStatus = 'Failed'
            Write-MigrationLog -Message "Edge profile '$($Profile.ProfileName)' had no data to export" -Level Warning
        }
    }
    catch {
        $Profile.ExportStatus = 'Failed'
        Write-MigrationLog -Message "Failed to export Edge profile '$($Profile.ProfileName)': $($_.Exception.Message)" -Level Error
    }
