<#
========================================================================================================
    Title:          Win11Migrator - Chrome Profile Importer
    Filename:       Import-ChromeProfile.ps1
    Description:    Imports Google Chrome bookmarks, extensions, settings, and history from a migration package.
...
<#
.SYNOPSIS
    Restores a Chrome browser profile from a migration package.
.DESCRIPTION
    Restores bookmarks and preferences to the Chrome profile directory. For extensions,
    generates an HTML page with links to the Chrome Web Store for easy reinstallation.
    Waits for Chrome to not be running before restoring files.
.PARAMETER Profile
    A BrowserProfile object for a Chrome profile.
.PARAMETER PackagePath
    Root of the migration package.
.PARAMETER WaitForBrowserClose
    If true, waits up to 60 seconds for Chrome to close before proceeding.
.OUTPUTS
...
function Import-ChromeProfile {
    ...
    if ($Profile.Browser -ne 'Chrome') {
        Write-MigrationLog -Message "Import-ChromeProfile called with non-Chrome profile: $($Profile.Browser)" -Level Warning
        $Profile.ExportStatus = 'Skipped'
        return $Profile
    }

    $profileName = $Profile.ProfileName -replace '[^\w\-\.]', '_'
    # PackagePath is the profile-specific directory (e.g. .../BrowserProfiles/Chrome_Default)
    $sourcePath = $PackagePath
    $targetProfilePath = $Profile.ProfilePath

    Write-MigrationLog -Message "Importing Chrome profile '$($Profile.ProfileName)' to $targetProfilePath" -Level Info

    if (-not (Test-Path $sourcePath)) {
        $Profile.ExportStatus = 'Failed'
        Write-MigrationLog -Message "Chrome profile package not found: $sourcePath" -Level Warning
        return $Profile
    }

    # If target profile path does not exist, try to construct it
    if (-not $targetProfilePath -or -not (Test-Path (Split-Path $targetProfilePath -Parent) -ErrorAction SilentlyContinue)) {
        $chromeUserData = Join-Path $env:LOCALAPPDATA 'Google\Chrome\User Data'
        $targetProfilePath = Join-Path $chromeUserData $Profile.ProfileName
    }

    try {
        # Wait for Chrome to close
        $waited = 0
        while ((Get-Process -Name 'chrome' -ErrorAction SilentlyContinue) -and $waited -lt $WaitTimeoutSeconds) {
            if ($waited -eq 0) {
                Write-MigrationLog -Message "Chrome is running. Waiting for it to close before restoring profile..." -Level Warning
            }
        ...
        if (Get-Process -Name 'chrome' -ErrorAction SilentlyContinue) {
            Write-MigrationLog -Message "Chrome is still running after $WaitTimeoutSeconds seconds. Attempting import anyway." -Level Warning
        }
        ...
        if (Test-Path $bookmarksFile) {
            # Backup existing bookmarks if present
            ...
            if (Test-Path $existingBookmarks) {
                ...
                Write-MigrationLog -Message "  Existing Chrome bookmarks backed up as $backupName" -Level Debug
            }

            Copy-Item -Path $bookmarksFile -Destination $existingBookmarks -Force -ErrorAction Stop
            Write-MigrationLog -Message "  Chrome bookmarks restored" -Level Debug
            $importedAny = $true
        }
        ...
        if (Test-Path $prefsFile) {
            ...
            Write-MigrationLog -Message "  Chrome preferences restored" -Level Debug
            $importedAny = $true
        }
        ...
        if (Test-Path $historyFile) {
            try {
                ...
                Write-MigrationLog -Message "  Chrome history restored" -Level Debug
                $importedAny = $true
            }
            catch {
                Write-MigrationLog -Message "  Chrome history restore failed (file may be locked): $($_.Exception.Message)" -Level Warning
            }
        }
...
<head>
    ...
    <title>Chrome Extensions - Reinstall</title>
    <style>
        ...
        .ext-list li a { color: #1a73e8; text-decoration: none; font-weight: 600; font-size: 16px; }
        .ext-list li a:hover { text-decoration: underline; }
        .ext-meta { color: #888; font-size: 13px; margin-top: 4px; }
    </style>
</head>
<body>
    <h1>Chrome Extensions from Previous Installation</h1>
    <p class="info">Click each link below to reinstall the extension from the Chrome Web Store.</p>
    <ul class="ext-list">
"@
                ...
                foreach ($ext in $extensionList) {
                    ...
                    $url  = if ($ext.WebStoreUrl) { $ext.WebStoreUrl } else { "https://chrome.google.com/webstore/detail/$($ext.Id)" }
...
"@
                ...
                Write-MigrationLog -Message "  Chrome extensions reinstall page created: $htmlPath ($(@($extensionList).Count) extensions)" -Level Debug
                $importedAny = $true
            }
            catch {
                Write-MigrationLog -Message "  Failed to generate Chrome extensions HTML: $($_.Exception.Message)" -Level Warning
            }
        }

        if ($importedAny) {
            $Profile.ExportStatus = 'Success'
            Write-MigrationLog -Message "Chrome profile '$($Profile.ProfileName)' import complete" -Level Success
        }
        else {
            $Profile.ExportStatus = 'Failed'
            Write-MigrationLog -Message "Chrome profile '$($Profile.ProfileName)' had no data to import" -Level Warning
        }
    }
    catch {
        $Profile.ExportStatus = 'Failed'
        Write-MigrationLog -Message "Failed to import Chrome profile '$($Profile.ProfileName)': $($_.Exception.Message)" -Level Error
    }
